...
async function mostrarHistorialVentas() {
  try {
    ...
    const contenedor = document.getElementById('historial-ventas');
    contenedor.innerHTML = `<div class="sin-contenido" style="color: red;">Error al cargar ventas: ${error.message}</div>`;
  }
}
...
async function mostrarControlCaja() {
  try {
    ...
    const contenedor = document.getElementById('control-caja');
    contenedor.innerHTML = `<div class="sin-contenido" style="color: red;">Error al cargar caja: ${error.message}</div>`;
  }
}
